...
**Build:** the `AccentColor` asset's actual colours (§13.1: light `#2E7D32`, dark `#66BB6A`). It has been empty since M0, so the app has been showing iOS's default blue everywhere; set both appearances in `Resources/Assets.xcassets/AccentColor.colorset` and check that buttons, toggles, checkmarks and chevrons turn green. Then `Shared/Palette.swift`; the rounded font at the root; the Plan rows, Tonight card, TODAY tag and outline, and blank-week card; the dice and row animations; the aisle headers, progress card, tick pop and All done card; the three empty screens; the coloured Settings tiles. No `@Model` changes.
- [ ] The accent is green (`#2E7D32` light, `#66BB6A` dark) everywhere the system blue used to show.
